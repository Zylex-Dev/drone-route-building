...
- **Система авторизации и личный кабинет:**  
  ...
  - Сохранение миссий в базу данных
  - Личный кабинет с списком сохранённых миссий
  - Загрузка и редактирование миссий
...
- **PostgreSQL + PostGIS** (порт 5432): База данных для хранения пользователей и миссий
- **Auth Service** (Python/FastAPI, порт 8000): Сервис регистрации, авторизации и управления миссиями
- **Route Service** (Node.js/Express, порт 3000): Сервис расчёта маршрутов полёта
- **Client** (Nginx, порт 80): Статический фронтенд приложения

Все сервисы объединены в Docker Compose и запускаются одной командой.
...
- **PostgreSQL 16** с расширением **PostGIS**:  
  Хранение пользователей и миссий с поддержкой геопространственных данных.

### Backend

- **Python/FastAPI** (Auth Service):  
  - Регистрация и авторизация пользователей (JWT токены)
  - CRUD операции для миссий
  - SQLAlchemy ORM + GeoAlchemy2 для работы с PostGIS
  - Валидация данных через Pydantic
  - Автоматическая генерация OpenAPI документации

- **Node.js/Express** (Route Service):  
  Расчёт оптимальных маршрутов полёта дронов.
